      ...
      real*8 par(21) !all measurements
...
!for xlink this value is probably too small
...
!production file list
 ...
 1414 continue!come back here sometimes while working the file
      ...
      ibinstart=ileft !data decides bins
 ...
 1313 continue!loop reading the file here, the previous read is done
      ...
      if(ileft.gt.ibinend.and.inred.lt.inplines)then!ok, there is no more data for ibin since file is sorted
         !right here i need to do the same work on the input file
...
c         if(ibinstart.ge.ibigstrt.and.ibinend.le.ibigend)then
!if a bin overlaps two bigg bins we should contribute to both.
!how to work that out?
         if(ibinstart.ge.ibigstrt.and.ibinstart.lt.ibigend)then
     ...
     .              factr*bigrat(j)/dble(nbres)!, bigrat(j)/dble(nbres) !no-scale
               else!don't NaN
               write(33,*) unk, ibigstrt, (j-0.5)*ishl, 
     .              0d0!, 0d0 !no-scale
               endif
     ...
     .              , factr*c(j)/dble(nbres)!, c(j)/dble(nbres) !this is no-scale
            enddo
            do k=idif+1,1000!shift to next windows
               c(k-(idif))=c(k)
            enddo
            do k=1000-idif,1000!should be dif+1?
            c(k)=0d0
            enddo
